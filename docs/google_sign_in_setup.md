...
## 5. Diagnóstico en Android

Si en Logcat aparece `SignInHubActivity` y la pantalla vuelve inmediatamente a la app sin mostrar cuentas, el flujo nativo de Google sí se abrió, pero pudo regresar sin cuenta seleccionada o con una sesión cacheada inválida. La app fuerza una limpieza local de sesión Google antes de llamar `signIn()` para mostrar el selector cuando Google lo permita.

Validaciones recomendadas en ese caso:

1. Usar un dispositivo/emulador con Google Play Services.
2. Confirmar que existe al menos una cuenta Google agregada al dispositivo.
3. Limpiar la app y reinstalar después de cambios OAuth:

```bash
flutter clean
flutter pub get
adb uninstall com.example.flips_app
flutter run --dart-define=GOOGLE_WEB_CLIENT_ID="TU_WEB_CLIENT_ID.apps.googleusercontent.com"
```

4. Si solo hay una cuenta Google en el dispositivo, Android puede seleccionar esa cuenta sin mostrar una lista larga.
5. Revisar el mensaje `Detalle:` mostrado por la app cuando Google lance una `PlatformException`; normalmente incluye códigos como `10` o `12500`.
